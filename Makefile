SERVICE = miscindexer
SERVICE_CAPS = MiscIndexer
SPEC_FILE = MiscIndexer.spec
URL = https://kbase.us/services/miscindexer
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build-test-script:
	...
	echo 'python -m nose --with-coverage --cover-package=$(SERVICE_CAPS),Utils --cover-html --cover-html-dir=/kb/module/work/test_coverage --nocapture  --nologcapture .' >> $(TEST_DIR)/$(TEST_SCRIPT_NAME)
